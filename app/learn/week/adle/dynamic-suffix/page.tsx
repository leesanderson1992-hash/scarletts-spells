...
export default async function DynamicSuffixPage(props: { searchParams?: Promise<{ child?: string; mode?: string }> }) {
  ...
  const preview = await previewDynamicAffixAssignment({ serviceClient: createServiceRoleClient(), childId: child.id, allowStagingProfiles, purpose: "readiness_preview" });

...
Whole-writing effects use
`WHOLE_WRITING_VERIFICATION_POLICY_V1_2026_09_08`,
`WHOLE_WRITING_REMEDIATION_POLICY_V1_2026_09_08`, and
`WHOLE_WRITING_MIXED_OUTCOME_POLICY_V1_2026_09_08` from their canonical
contract owners.
...
- **Snapshot recurrence** retains every occurrence but derives at most one
  positive and one causal-negative performance for a snapshot, canonical or
  intended word, micro-skill, and outcome.
...
| Later independent success after a previous Review or authentic-writing failure | Source-specific: isolated, contextual, or authentic | +1 only if still new | Marks coverage for the later event's source | Counts according to the later environment | Resolves proficiency-only instability for the same intended word and causal skill; history remains | Does not delete the earlier negative | Source-specific reward effect | Scheduler state changes only if the Word Progression authority recognises the event | Eligible new event; repair, answer-visible and same-snapshot use are excluded |
...
| Verified authentic occurrence that is correctly formed but contextually wrong | `AUTHENTIC_WRITING` | +1 if new only for unrelated governed orthographic skills | Marks coverage only for those unrelated skills | No positive projection to the causal context skill | Opens proficiency-only instability for the intended word and causal context skill | One governed contextual negative; the same skill cannot also receive a positive | No authentic-use reward for the invalid occurrence | Creates/reactivates a need only through existing governed intake policy | Mixed projection under `WHOLE_WRITING_MIXED_OUTCOME_POLICY_V1_2026_09_08` |
...
| Repeated correct use of the same word in one snapshot | Source-specific | No additional breadth | No additional band/group coverage | No additional distinct-word transfer count | Snapshot occurrences dedupe to one positive performance; preserve raw lineage | At most one causal-negative performance for the same word/skill/outcome | Reward dedupe remains Word Treasure + task-field scoped | Cannot resolve another occurrence's error in that snapshot | Raw events retained; snapshot aggregate dedupes |
...
| Parent submits a source-scoped whole-writing attestation | `VERIFICATION_TRANSITION` | Activates only included eligible occurrences if all other gates pass; no extra event | Activates each included event's coverage | Activates transfer according to each occurrence's source classification | Uses each occurrence timestamp; verification time does not replace it | Exclusions and unresolved findings remain blocked | Forward consumers apply their own policy; historical restrictions remain | Allows evidence-informed state change; verification itself is not performance | Included/excluded occurrence fingerprint and assistance/authorship facts required |

...
| Contextual Transfer Review: target spelled correctly in the original creative/problem-solving writing | `CONTEXTUAL_TRANSFER` | +1 if new | Marks group/band; can satisfy transfer-at-complexity coverage | +1 distinct contextual word for every governed demonstrated skill | Counts as independent transfer, observation day, and elapsed span | None | Under `WORD_TREASURE_AUTHENTIC_USE_V2`, may increment the same word once per completed governed task after Forge when the authored occurrence, answer-hidden, no-repair, and contextual-validity gates pass; retains prompted Review provenance and needs no second parent approval | Supports `CONSOLIDATION` and, with other gates, `MAINTENANCE` | Strong eligible positive |
...
- Review writing already provides a meaningful mixed writing challenge, but
  ...
  `ISOLATED_RETRIEVAL`.
- Prompted Review writing remains contextual rather than authentic proficiency.
  Its separate V2 Gold Bar admission is explicit, policy-versioned,
  source-distinguished, and task-deduplicated; an `authentic_use` name alone is
  never sufficient.

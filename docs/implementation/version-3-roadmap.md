...
| 7P | live pilot foundation: read-only ADLE composer preview, real child identity preflight, explicit guarded generation, `/learn/week/adle` read-only on child load, child nav routed to ADLE, legacy `learning_items` Daily Practice path documented/retired for child pilot | Phase 13 bridge | **complete 2026-07-09**; real-child assignment was previewed, intentionally generated, opened, completed, and DB-verified |
| 7R | attempt capture and evidence classification integrity: item-level ADLE attempt ledger, guided/reflection attempt submission, first-exposure mistakes stored as non-punitive lesson evidence, scheduled review outcomes kept review-only, authentic-use kept free-writing-only, assignment header completion derived from item completion | Phase 13 bridge | **complete 2026-07-09**; hosted migration deployed and a wrong scheduled-review attempt wrote the correct attempt ledger + `review_fail` outcome with no authentic-use event |
| 7S | reflection recall gate: reflection repair shows the correct spelling for teaching, then requires the child to hide the word before the retry input appears, preserving reflection evidence quality | Phase 13 bridge | **complete 2026-07-10**; next step is 7-UI template redesign |
| 7-UI (NEXT AFTER 7P/7R/7S PROOF) | template UI/UX redesign: rework the child experience of each activity template — the owner is **not happy with the current per-template UI/UX**. Starts at its own fresh planning phase. The 7a registry is the drop-in seam (`templateKey → component`); the warm prompt shells + the tier map are what this slice replaces/upgrades | Phase 13 (child) | planned — unblocked after the real-child ADLE assignment, item-level attempt capture, scheduled-review miss, and reflection recall-gate proofs |

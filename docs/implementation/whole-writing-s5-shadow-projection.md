...
`writing_shadow_current_occurrence_evidence` deterministically selects the newest
eligible receipt for each occurrence. Authority-event sequence takes precedence
over run completion time, so an older or ordinary replay cannot displace a newer
publication or withdrawal interpretation. Partial E1 replays retain the current
receipts for unaffected occurrences in the same snapshot. Historical reads
address all batches directly.

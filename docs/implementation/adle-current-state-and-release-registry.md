...
| Remaining first-impression suffix lessons | `D4_MOR_SUFFIXES_LY`, `D4_MOR_SUFFIXES_TION`, `D4_MOR_SUFFIXES_SION` | `awaiting_content` | none | These are the only outstanding suffix first-impression profiles. `-tion` / `-sion` should be planned together because their spelling and meaning contrasts interact. |

...
import type {
  WritingEngineStage4cAuthenticWritingIssuePromotionBuildResult,
  WritingEngineStage4cAuthenticWritingIssuePromotionInput,
  WritingEngineStage4cAuthenticWritingIssuePromotionPersistenceInput,
  WritingEngineStage4cAuthenticWritingIssuePromotionResult,
  WritingEngineStage4cDurableIssueTruth,
} from "./stage4c-authentic-writing-issue-types";

function isPromotableDecision(
  result: WritingEngineStage4bAuthenticSubmissionVerificationResult,
): result is WritingEngineStage4bAuthenticSubmissionVerificationResult & {
...
function requirePromotableLineage(
  input: WritingEngineStage4cAuthenticWritingIssuePromotionInput,
) {
  ...
  if (!sourceRef.taskSubmissionId) {
    throw new Error(
      "Punctuation authentic-writing issue promotion requires a task submission id.",
    );
  }

  if (!childAttemptText) {
    throw new Error(
      "Punctuation authentic-writing issue promotion requires preserved child attempt text.",
    );
  }

  if (!targetText) {
    throw new Error(
      "Punctuation authentic-writing issue promotion requires preserved target text.",
    );
  }

  if (positionStart === null || positionEnd === null) {
    throw new Error(
      "Punctuation authentic-writing issue promotion requires preserved source-span lineage.",
    );
  }

  if (!input.verificationResult.parentVerifiedTruth?.microSkillKey) {
    throw new Error(
      "Punctuation authentic-writing issue promotion requires a verified micro-skill key.",
    );
  }
...
function buildDurableIssueTruth(
  ...
  if (!isPromotableDecision(input.verificationResult)) {
    throw new Error(
      "Only accepted and overridden punctuation authentic-writing outcomes may create durable issue truth.",
    );
  }
...
export function buildPunctuationAuthenticWritingIssuePromotion(
  input: WritingEngineStage4cAuthenticWritingIssuePromotionInput,
): WritingEngineStage4cAuthenticWritingIssuePromotionBuildResult {
...
export function promotePunctuationVerifiedOutcome(
  input: WritingEngineStage4cAuthenticWritingIssuePromotionInput,
): WritingEngineStage4cAuthenticWritingIssuePromotionResult {
  return buildPunctuationAuthenticWritingIssuePromotion(input).result;
}

export async function persistPunctuationAuthenticWritingIssuePromotion(
  input: WritingEngineStage4cAuthenticWritingIssuePromotionPersistenceInput,
): Promise<WritingEngineStage4cAuthenticWritingIssuePromotionResult> {
  const built = buildPunctuationAuthenticWritingIssuePromotion(

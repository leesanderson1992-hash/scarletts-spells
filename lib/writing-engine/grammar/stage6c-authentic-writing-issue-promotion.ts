...
import type {
  WritingEngineStage6cAuthenticWritingIssuePromotionBuildResult,
  WritingEngineStage6cAuthenticWritingIssuePromotionInput,
  WritingEngineStage6cAuthenticWritingIssuePromotionPersistenceInput,
  WritingEngineStage6cAuthenticWritingIssuePromotionResult,
  WritingEngineStage6cDurableIssueTruth,
} from "./stage6c-authentic-writing-issue-types";

function isPromotableDecision(
  result: WritingEngineStage6bAuthenticSubmissionVerificationResult,
): result is WritingEngineStage6bAuthenticSubmissionVerificationResult & {
...
function requirePromotableLineage(
  input: WritingEngineStage6cAuthenticWritingIssuePromotionInput,
) {
  ...
  if (!sourceRef.taskSubmissionId) {
    throw new Error(
      "Grammar/proofreading authentic-writing issue promotion requires a task submission id.",
    );
  }

  if (!childAttemptText) {
    throw new Error(
      "Grammar/proofreading authentic-writing issue promotion requires preserved child attempt text.",
    );
  }

  if (!targetText) {
    throw new Error(
      "Grammar/proofreading authentic-writing issue promotion requires preserved target text.",
    );
  }

  if (positionStart === null || positionEnd === null) {
    throw new Error(
      "Grammar/proofreading authentic-writing issue promotion requires preserved source-span lineage.",
    );
  }

  if (!input.verificationResult.parentVerifiedTruth?.microSkillKey) {
    throw new Error(
      "Grammar/proofreading authentic-writing issue promotion requires a verified micro-skill key.",
    );
  }
...
function buildDurableIssueTruth(
  ...
  if (!isPromotableDecision(input.verificationResult)) {
    throw new Error(
      "Only accepted and overridden grammar/proofreading authentic-writing outcomes may create durable issue truth.",
    );
  }
...
export function buildGrammarProofreadingAuthenticWritingIssuePromotion(
  input: WritingEngineStage6cAuthenticWritingIssuePromotionInput,
): WritingEngineStage6cAuthenticWritingIssuePromotionBuildResult {
...
export function promoteGrammarProofreadingVerifiedOutcome(
  input: WritingEngineStage6cAuthenticWritingIssuePromotionInput,
): WritingEngineStage6cAuthenticWritingIssuePromotionResult {
  return buildGrammarProofreadingAuthenticWritingIssuePromotion(input).result;
}

export async function persistGrammarProofreadingAuthenticWritingIssuePromotion(
  input: WritingEngineStage6cAuthenticWritingIssuePromotionPersistenceInput,
): Promise<WritingEngineStage6cAuthenticWritingIssuePromotionResult> {
  const built = buildGrammarProofreadingAuthenticWritingIssuePromotion(

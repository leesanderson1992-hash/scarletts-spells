...
export function compileBaseWordCanonicalIntakeRouteFacts(input: {
  ...
  const activations = input.activations.filter((activation) =>
    ...
    baseWordFamilyAuthorityAppliesToMicroSkill(activation.family, activation.microSkillKey) &&
  ...
  for (const activation of activations) {
    ...
    for (const family of activation.family.families) {
      for (const member of family.members) {
        ...
        const exactFamilyRelationship =
          baseWordFamilyMemberAppliesToMicroSkill(activation.family, member, activation.microSkillKey) &&
          (activation.family.schemaVersion === 2 || ("memberRole" in member && member.memberRole === "authentic_target"));
        const exactAuthenticMember =
          exactFamilyRelationship &&
          ...
          evidence: [
            {
              ...
              status: `${baseWordFamilyMemberStructuralRole(member)}:${member.assignmentEligible ? "assignment_eligible" : "not_assignment_eligible"}`,
            },
            {

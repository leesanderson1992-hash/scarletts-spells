...
export type BaseWordFamilyLessonStage = "intro" | "families" | "word_sums" | "controlled" | "dictation" | "reflect";
...
const STAGES: BaseWordFamilyLessonStage[] = ["intro", "families", "word_sums", "controlled", "dictation", "reflect"];
...
export function normaliseBaseWordFamilyResume(value: unknown, payload: BaseWordFamilyLessonSnapshotV1): BaseWordFamilyResumeState | null {
  ...
  if (!STAGES.includes(state.stage as BaseWordFamilyLessonStage) || !Number.isInteger(state.familyIndex) || !Number.isInteger(state.controlledIndex) || !Number.isInteger(state.dictationIndex) || typeof state.sentenceChecked !== "boolean" || typeof state.reflectionText !== "string" || state.reflectionText.length > 2000 || !recordOfStrings(state.controlledAttempts) || !recordOfBooleans(state.controlledChecked) || !recordOfStrings(state.sentenceAttempts)) return null;
  if (state.familyIndex! < 0 || state.familyIndex! >= payload.familySections.length || state.controlledIndex! < 0 || state.controlledIndex! >= payload.independentWords.length || state.dictationIndex! < 0 || state.dictationIndex! >= payload.independentWords.length) return null;
  ...
  return state as BaseWordFamilyResumeState;
}

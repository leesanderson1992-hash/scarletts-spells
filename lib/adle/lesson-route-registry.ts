...
export type AdleLessonRouteKey =
  | "base_word_family_v1"
  | "generic_first_exposure_v1"
  | "review_v1";
...
export type AdleRouteCapability =
  | "first_exposure"
  | "review"
  | "reteach"
  | "transfer";
...
export interface AdleLessonRouteDefinition {
  ...
  compatibleMicroSkillPrefixes: readonly string[];
...
function bounded(
  input: AdleLessonRouteReadinessInput,
  definition: Omit<AdleLessonRouteDefinition, "validateReadiness">,
): AdleCurriculumReadinessResult {
  const blockers: AdleCurriculumReadinessResult["blockers"] = [];
  const compatible =
    definition.compatibleMicroSkillKeys?.includes(input.microSkillKey) ||
    definition.compatibleMicroSkillPrefixes.some((prefix) =>
      input.microSkillKey.startsWith(prefix),
    );
  if (!definition.payloadVersions.includes(input.payloadVersion))
  ...
  if (!input.sharedCurriculumComplete)
    blockers.push({ reason: "missing_shared_curriculum", evidence: {} });
  if (!input.routeCurriculumComplete)
    blockers.push({ reason: "missing_route_curriculum", evidence: {} });
  if (input.authenticTargetCount < definition.authenticTargets.min || input.authenticTargetCount > definition.authenticTargets.max)
    blockers.push({ reason: "invalid_authentic_target_count", evidence: { authenticTargetCount: input.authenticTargetCount } });
  if (input.practiceWordCount < definition.practiceWords.min || input.practiceWordCount > definition.practiceWords.max)
    blockers.push({ reason: "invalid_word_count", evidence: { practiceWordCount: input.practiceWordCount } });
  if (input.transferWordCount < definition.transferWords.min || input.transferWordCount > definition.transferWords.max)
...
function define(
  value: Omit<AdleLessonRouteDefinition, "validateReadiness">,
): AdleLessonRouteDefinition {
  return { ...value, validateReadiness: (input) => bounded(input, value) };
}

export const ADLE_LESSON_ROUTE_REGISTRY: ReadonlyMap<
  AdleLessonRouteKey,
  AdleLessonRouteDefinition
> = new Map([
  ...
  ["generic_first_exposure_v1", define({
    lessonRouteKey: "generic_first_exposure_v1",
    payloadVersions: [1],
    compatibleMicroSkillPrefixes: ["D4_"],
    capabilities: ["first_exposure", "reteach", "transfer"],
    activityTemplateKeys: ["MICRO_READ_ONLY_INTRO", "LESSON_WORDS_INTRO", "CONTROLLED_SPELLING", "DICTATION_NO_IMAGE", "MUST_USE_FREEWRITING"],
    authenticTargets: { min: 1, max: 5 },
    practiceWords: { min: 5, max: 5 },
    transferWords: { min: 0, max: 5 },
    requiredSharedFields: ["canonical_word", "micro_skill", "teaching_content", "word_support"],
    requiredRouteFields: ["lesson_words", "guided_progression", "production_prompt"],
  })],
  ["review_v1", define({
    lessonRouteKey: "review_v1",
    payloadVersions: [1],
    compatibleMicroSkillPrefixes: ["D4_"],
    capabilities: ["review", "transfer"],
    activityTemplateKeys: ["REVIEW_QUICK_SORT", "REVIEW_DICTATION", "DICTATION_SENTENCE_CONTEXT", "REVIEW_MUST_USE_WRITING", "ERROR_REFLECTION_CUE"],
    authenticTargets: { min: 1, max: 25 },
    practiceWords: { min: 1, max: 25 },
    transferWords: { min: 0, max: 25 },
    requiredSharedFields: ["canonical_word", "micro_skill", "teaching_content", "word_support"],
    requiredRouteFields: ["schedule", "production_requirement"],
  })],
]);

export function getAdleLessonRouteDefinition(
  key: string,
): AdleLessonRouteDefinition | null {

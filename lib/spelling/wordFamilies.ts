...
} as const satisfies Partial<Record<WordFamilyId, readonly string[]>>;
...
const HOMOPHONE_GROUP_BY_SET: Partial<Record<WordFamilyId, WordFamilyId>> = {
...
const LEGACY_FAMILY_ID_MAP: Record<string, string> = {
...
export const WORD_FAMILIES: WordFamily[] = [
  ...
  {
    id: "final_le_patterns",
    label: "Final -le / -el / -al patterns",
    description: "Words where the final consonant + vowel ending pattern must be remembered carefully.",
    graphemes: ["le", "el", "al"],
    practiceWords: ["circle", "table", "little", "camel", "metal", "total"],
  },
  {
  ...
  {
    id: "drop_keep_final_e_suffixes",
    label: "Drop or keep final e before suffixes",
    description: "Words that need the child to decide whether the final e stays or drops before a suffix.",
    graphemes: ["e + suffix"],
    practiceWords: ["making", "hoped", "hoping", "useful", "changeable", "shining"],
  },
  {
    id: "change_y_to_i",
    label: "Change y to i",
  ...
  {
    id: "common_prefixes",
    label: "Common prefixes",
    description: "Words that keep a recognisable spelling when common prefixes are added.",
    graphemes: ["un", "re", "dis", "mis", "pre"],
    practiceWords: ["unhappy", "rewrite", "disagree", "misbehave", "preview", "prepare"],
  },
  {
    id: "common_suffixes",
    label: "Common suffixes",
    description: "Words built with common endings such as -ed, -ing, -ly, -ness and -ment.",
    graphemes: ["ed", "ing", "ly", "ness", "ment"],
    practiceWords: ["helped", "helping", "slowly", "darkness", "movement", "careless"],
  },
  {
    id: "root_family_preservation",
    label: "Root family preservation",
    description: "Words that keep the root spelling visible as they grow into related words.",
    graphemes: [],
    practiceWords: ["sign", "signal", "signature", "heal", "health", "musician"],
  },
  {
    id: "tion_sion_suffixes",
    label: "-tion / -sion endings",
    description: "Words that use common noun-building endings such as -tion and -sion.",
    graphemes: ["tion", "sion"],
    practiceWords: ["station", "fiction", "action", "decision", "division", "expression"],
  },
  {
...
function normaliseHomophoneWord(word: string) {
  return word.trim().toLowerCase().replace(/['’]/g, "");
}

export function getHomophoneWordsForFamily(familyId: WordFamilyId) {
  const setWords =
    familyId in HOMOPHONE_SET_WORDS
      ? HOMOPHONE_SET_WORDS[familyId as keyof typeof HOMOPHONE_SET_WORDS]
      : [];
  const groupWords =
    familyId in HOMOPHONE_GROUP_WORDS
      ? HOMOPHONE_GROUP_WORDS[familyId as keyof typeof HOMOPHONE_GROUP_WORDS]
      : [];
...
export function findHomophoneSetFamilyForWord(word: string): WordFamily | undefined {
  const normalisedWord = normaliseHomophoneWord(word);
  const setFamilyId = Object.entries(HOMOPHONE_SET_WORDS).find(([, words]) =>
    words.map(normaliseHomophoneWord).includes(normalisedWord),
  )?.[0] as WordFamilyId | undefined;

  return setFamilyId ? getWordFamilyById(setFamilyId) : undefined;
}

export function findHomophoneSetFamilyForWords(
  leftWord: string,
  rightWord: string,
  ...
  const setFamilyId = Object.entries(HOMOPHONE_SET_WORDS).find(([, words]) => {
    ...
    return (
      normalisedWords.includes(normalisedLeft) &&
...
export function findHomophoneGroupFamilyForWord(word: string): WordFamily | undefined {
  const normalisedWord = normaliseHomophoneWord(word);
  const groupFamilyId = Object.entries(HOMOPHONE_GROUP_WORDS).find(([, words]) =>
    words.map(normaliseHomophoneWord).includes(normalisedWord),
  )?.[0] as WordFamilyId | undefined;
...
export function getRelatedHomophoneGroupFamilyId(
  familyId: WordFamilyId | null | undefined,
) {
  if (!familyId) {
    return null;
  }
...
export function isWordFamilyId(value: string): value is WordFamilyId {
  return WORD_FAMILIES.some((family) => family.id === value);
}

export function normaliseWordFamilyId(
...
export function asWordFamilyId(value: string | null | undefined): WordFamilyId | null {
...
export function getWordFamilyById(id: WordFamilyId): WordFamily | undefined {
  return WORD_FAMILIES.find((family) => family.id === id);
}

export function matchesWordFamily(word: string, familyId: WordFamilyId): boolean {
  switch (familyId) {
    case "ai-ay":
    ...
    case "drop_keep_final_e_suffixes":
      return /(ing|ed|er|est|able|ably)$/.test(word) && /e/.test(word);
    case "change_y_to_i":
      return /(ied|ies|ier|iest)$/.test(word);
    case "common_prefixes":
      return /^(un|re|dis|mis|pre)/.test(word);
    case "common_suffixes":
      return /(ed|ing|ly|ness|ment|less|ful)$/.test(word);
    case "root_family_preservation":
      return /(sign|signal|signature|heal|health|musician|music|musical)$/.test(word);
    case "tion_sion_suffixes":
      return /(tion|sion|cian)$/.test(word);
    ...
    case "final_le_patterns":
      return /[bcdfghjklmnpqrstvwxyz](le|el|al)$/.test(word);
...
export function findWordFamilyForWord(word: string): WordFamily | undefined {
  return WORD_FAMILIES.find((family) => matchesWordFamily(word, family.id));
}

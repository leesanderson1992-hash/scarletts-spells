-- ADLE C2B.6: additive, feature-off coexistence and governed policy cutover.
-- The TypeScript reducers remain the only scheduler transition authority.
-- This migration does not activate/default the target policy or cut over data.

create or replace function public.apply_adle_review_policy_cutover_c2b6(
  p_approved_child_id uuid,
  p_reviewed_preview_fingerprint text,
...
declare
  ...
  v_ids uuid[];
begin
  if p_approved_child_id is null
    or p_reviewed_preview_fingerprint !~ '^[a-f0-9]{64}$'
    or nullif(btrim(p_approval_reference), '') is null
    or jsonb_typeof(p_candidates) <> 'array'
    or jsonb_array_length(p_candidates) < 1
    or jsonb_array_length(p_candidates) > 19
  then raise exception 'adle_c2b6_cutover_envelope_malformed'; end if;

  select array_agg((candidate->>'scheduleWordId')::uuid order by candidate->>'scheduleWordId')
  into v_ids from jsonb_array_elements(p_candidates) candidate;
  if cardinality(v_ids) <> (select count(distinct value) from unnest(v_ids) value)
  then raise exception 'adle_c2b6_cutover_candidate_duplicate'; end if;

  -- Stable locks serialize cutover against Review, controlled graduation and
  -- another cutover. No row can be partially converted.
  perform 1 from public.adle_review_schedule_words word
  where word.id = any(v_ids) order by word.id for update;
  if (select count(*) from public.adle_review_schedule_words where id = any(v_ids))
      <> cardinality(v_ids)
  then raise exception 'adle_c2b6_cutover_candidate_missing'; end if;

  for v_candidate in
    select value from jsonb_array_elements(p_candidates) order by value->>'scheduleWordId'
  loop
    if v_candidate - array[
      'scheduleWordId','expectedRevision','expectedPolicyVersion',
      'expectedStateShapeVersion','expectedMembershipStatus',
      'expectedIntervalIndex','expectedDueOn','expectedLast28DayReviewOn',
      'expectedLastReviewCompletedOn','expectedLastReviewCompletedAt',
      'toState','idempotencyKey','sourceFingerprint'
    ]::text[] <> '{}'::jsonb
      or (v_candidate->>'expectedRevision') !~ '^[0-9]+$'
      or (v_candidate->>'expectedIntervalIndex') !~ '^[0-4]$'
      or nullif(btrim(v_candidate->>'idempotencyKey'), '') is null
      or v_candidate->>'sourceFingerprint' !~ '^[a-f0-9]{64}$'
      or jsonb_typeof(v_candidate->'toState') <> 'object'
    then raise exception 'adle_c2b6_cutover_candidate_malformed'; end if;

    select * into v_word from public.adle_review_schedule_words
    where id = (v_candidate->>'scheduleWordId')::uuid;
    if v_word.child_id <> p_approved_child_id then
      raise exception 'adle_c2b6_cutover_child_scope_conflict'; end if;

    select * into v_existing
    from public.adle_review_schedule_transition_events event
    where event.schedule_word_id = v_word.id
      and event.idempotency_key = v_candidate->>'idempotencyKey';
    if found then
      if v_existing.transition_kind <> 'POLICY_CUTOVER_APPLIED'
        or v_existing.source_fingerprint <> v_candidate->>'sourceFingerprint'
        or v_existing.cutover_approval_reference <> p_approval_reference
      then raise exception 'adle_c2b6_cutover_idempotency_conflict'; end if;
      v_replayed := v_replayed + 1;
      continue;
    end if;

    if v_word.row_status <> 'active'
      or v_word.word_schedule_policy_version <> v_candidate->>'expectedPolicyVersion'
      or v_word.word_schedule_version <> v_candidate->>'expectedStateShapeVersion'
      ...
      or v_word.membership_status <> v_candidate->>'expectedMembershipStatus'
      or v_word.membership_status <> 'scheduled'
      or v_word.word_interval_index <> (v_candidate->>'expectedIntervalIndex')::integer
      or v_word.word_next_due_on is distinct from (v_candidate->>'expectedDueOn')::date
      or v_word.word_schedule_transition_count <> (v_candidate->>'expectedRevision')::bigint
      or v_word.catch_up_stage <> 0 or v_word.next_retest_due_on is not null
      or v_word.failed_review_on is not null or v_word.pre_retirement_check_due_on is not null
      or v_word.consecutive_independent_failures is not null
      or v_word.failure_episode_id is not null
      or v_word.last_28_day_review_on is distinct from nullif(v_candidate->>'expectedLast28DayReviewOn','')::date
      or v_word.word_last_review_completed_on is distinct from nullif(v_candidate->>'expectedLastReviewCompletedOn','')::date
      or v_word.word_last_review_completed_at is distinct from nullif(v_candidate->>'expectedLastReviewCompletedAt','')::timestamptz
    then raise exception 'adle_c2b6_cutover_preview_drift'; end if;
    ...
    v_from_state := jsonb_build_object(
      ...
      'stateRevision', v_word.word_schedule_transition_count
    );
    v_to_state := v_candidate->'toState';
    if v_to_state - array[
      ...
      or v_to_state->>'membershipStatus' <> 'scheduled'
      or (v_to_state->>'wordIntervalIndex')::integer <> v_word.word_interval_index
      or (v_to_state->>'wordNextDueOn')::date <> v_word.word_next_due_on
      or (v_to_state->>'consecutiveIndependentFailures')::integer <> 0
      or v_to_state->>'failureEpisodeId' is not null
      or v_to_state->>'preRetirementCheckDueOn' is not null
      or nullif(v_to_state->>'last28DayReviewOn','')::date is distinct from v_word.last_28_day_review_on
      or nullif(v_to_state->>'wordLastReviewCompletedOn','')::date is distinct from v_word.word_last_review_completed_on
      or nullif(v_to_state->>'wordLastReviewCompletedAt','')::timestamptz is distinct from v_word.word_last_review_completed_at
    then raise exception 'adle_c2b6_cutover_to_state_conflict'; end if;

    v_envelope := jsonb_build_object(
      'scheduleWordId', v_word.id,
      'reviewedPreviewFingerprint', p_reviewed_preview_fingerprint,
      'approvalReference', p_approval_reference,
      'idempotencyKey', v_candidate->>'idempotencyKey',
      'expectedStateRevision', v_word.word_schedule_transition_count,
    ...
    if v_fingerprint <> v_candidate->>'sourceFingerprint'
    then raise exception 'adle_c2b6_cutover_fingerprint_conflict'; end if;
    ...
    where id = v_word.id
      and word_schedule_transition_count = (v_candidate->>'expectedRevision')::bigint;
    if not found then raise exception 'adle_c2b6_cutover_stale_revision'; end if;

    insert into public.adle_review_schedule_transition_events(
      schedule_word_id,child_id,canonical_word_id,schedule_policy_version,
      state_shape_version,transition_kind,cutover_approval_reference,
      idempotency_key,expected_state_revision,applied_state_revision,
      from_state,to_state,transition_reason,reducer_version,source_fingerprint
    ) values (
      v_word.id,v_word.child_id,v_word.canonical_word_id,
      'ADLE_SPACED_REVIEW_REGRESSION_V1','adle_review_per_word_schedule_v2',
      'POLICY_CUTOVER_APPLIED',p_approval_reference,
      v_candidate->>'idempotencyKey',v_word.word_schedule_transition_count,
      v_word.word_schedule_transition_count + 1,v_from_state,v_to_state,
      'POLICY_CUTOVER_APPROVED_CLEAN_SCHEDULED',
      'POLICY_CUTOVER_NO_LEARNER_EVENT_V1',v_fingerprint
    );
    v_applied := v_applied + 1;
  end loop;

  return jsonb_build_object(
    'status', case when v_applied = 0 then 'already_applied' else 'applied' end,
    'appliedCount', v_applied, 'replayedCount', v_replayed,
    'reviewedPreviewFingerprint', p_reviewed_preview_fingerprint
  );
end;
$$;

revoke all on function public.apply_adle_review_policy_cutover_c2b6(uuid,text,text,jsonb)
  from public, anon, authenticated;
grant execute on function public.apply_adle_review_policy_cutover_c2b6(uuid,text,text,jsonb)
  to service_role;

comment on function public.apply_adle_review_policy_cutover_c2b6(uuid,text,text,jsonb) is
  'C2B.6 exact-preview, revision-locked v1-to-v2 policy pin. It does not execute a learner-performance reducer event.';

-- Versioned R6 assignment persistence for a set containing at least one v2
-- target. The released v1-only RPC remains unchanged and is still used for
-- every v1-only session. This function verifies the TypeScript-selected due
-- set; it does not calculate a scheduler transition.
  ...
  with due_words as (
    select word.id,word.taught_on,word.canonical_word_id,
      case
        ...
        when word.word_schedule_version='adle_review_per_word_schedule_v2' and word.membership_status in ('scheduled','next_day_recovery') and word.word_interval_index<5 then word.word_next_due_on
    ...
    if not found or v_word.child_id<>p_child_id or v_word.row_status<>'active'
      ...
      or not (
        ...
        or (v_word.word_schedule_version='adle_review_per_word_schedule_v2'
          ...
          and v_word.word_interval_index<5 and v_word.word_next_due_on=v_due_on
          and ((v_due_kind='scheduled_review' and v_word.membership_status='scheduled')
            or (v_due_kind='next_day_recovery' and v_word.membership_status='next_day_recovery')))
      )
...
-- Read-only governed completion instant used to build reducer plans. The
-- finalizer revalidates this result after taking its locks.

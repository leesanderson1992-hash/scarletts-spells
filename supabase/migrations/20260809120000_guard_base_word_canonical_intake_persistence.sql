...
-- Guard Base Word canonical-intake route attribution at the database boundary.
-- This forward replacement preserves the complete BW-0 lifecycle while a
-- trailing defaulted activation argument keeps pre-BW-1 callers compatible.
...
begin
  ...
  if v_is_base_word then
    ...
    if p_route_activation_id is null then
      raise exception 'Base Word candidate requires an exact route activation';
    end if;
    perform 1
    ...
    join public.micro_skill_catalog skill
      on skill.micro_skill_key = activation.micro_skill_key
    join public.canonical_teaching_dictionary_import_batches import_batch
      on import_batch.id = manifest.import_batch_id
    ...
    where activation.id = p_route_activation_id
      and activation.micro_skill_key = p_micro_skill_key
      and activation.lesson_route_key = 'base_word_family_v1'
      ...
      and import_batch.batch_status = 'applied'
    ...
    for key share of activation, manifest, import_batch, skill, family, member;
    if not found then
      raise exception 'Base Word candidate is not an exact activation-bound authentic target';
    end if;
    v_route_id := 'base_word_lab';
    v_route_version := 'v2';
  elsif v_is_affix then
  ...
  insert into public.adle_canonical_intake_candidates (
    ...
    encode(extensions.digest(concat_ws(
      ...
      v_route_id,
      v_route_version,
      coalesce(p_route_activation_id::text, '')
    ), 'sha256'), 'hex'),
    timezone('utc', now()),
    v_learning_item_id,
    timezone('utc', now()),

...
function normaliseToken(value: string) { return value.replace(/[^a-z']/gi, ""); }
...
export function DiffReveal(props: { attempt: string; expected: string; splitPoints?: number[]; mode?: "word" | "sentence" }) {
  if (props.mode === "sentence") {
    ...
    const needsInitialCapital = expectedTokens[0] !== undefined && attemptedTokens[0] !== undefined && expectedTokens[0][0] !== attemptedTokens[0][0];
    const allWordsMatch = attemptedTokens.length === expectedTokens.length && matchedExpected.size === expectedTokens.length;
    const feedback = needsInitialCapital
      ? "Remember: a sentence starts with a capital letter."
      : allWordsMatch
        ? "Your sentence matches."
  ...
  const attempt = props.attempt; const expected = props.expected; const attemptNormalised = attempt.toLocaleLowerCase("en-GB"); const expectedNormalised = expected.toLocaleLowerCase("en-GB"); const max = Math.max(attempt.length, expected.length); const split = new Set(props.splitPoints ?? []);
  return <div className="rounded-2xl border border-emerald-300/30 bg-emerald-50 p-4 text-slate-950"><p className="text-xs font-bold uppercase tracking-wide text-emerald-800">Compare the word</p><div className="mt-3 flex flex-wrap justify-center gap-1" aria-label={`You wrote ${props.attempt}. The expected word is ${props.expected}.`}>{Array.from({ length: max }).map((_, index) => <span key={index} className="inline-flex items-center"><span className={`grid h-10 min-w-8 place-items-center rounded-lg font-black ${attemptNormalised[index] === expectedNormalised[index] ? "bg-emerald-200 text-emerald-950" : "animate-pulse bg-amber-200 text-amber-950 motion-reduce:animate-none"}`}>{attempt[index] ?? "_"}<span className="sr-only">{attemptNormalised[index] === expectedNormalised[index] ? "matches" : `compare with ${expected[index] ?? "nothing"}`}</span></span>{split.has(index + 1) ? <span aria-hidden="true" className="mx-1 text-cyan-700">|</span> : null}</span>)}</div><p className="mt-3 text-center text-sm font-semibold">{attemptNormalised === expectedNormalised ? "You kept both word parts." : "Look at the glowing comparison and notice what changed."}</p></div>;
}

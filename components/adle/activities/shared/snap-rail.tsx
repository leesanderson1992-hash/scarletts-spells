...
export function SnapRail(props: { tiles: RailTile[]; expectedIds: string[]; fixedTiles?: RailTile[]; label: string; recallNeutral?: boolean; muted?: boolean; onComplete?: (word: string) => void; onInvalid?: (ids: string[]) => void }) {
  ...
  function place(id: string) {
    ...
    if (next.length === props.expectedIds.length) {
      if (next.every((value, index) => value === props.expectedIds[index])) {
        setPlaced(next); setAnnouncement("Word parts joined successfully"); playInteractionSound("fusion", props.muted); props.onComplete?.([...next.map((value) => props.tiles.find((tile) => tile.id === value)?.text ?? ""), ...(props.fixedTiles ?? []).map((tile) => tile.text)].join(""));

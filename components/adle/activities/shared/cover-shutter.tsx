...
export function CoverShutter(props: CoverShutterProps) {
  ...
  async function closeShutter() {
    if (state !== "look" || coverRequested.current) return;
    coverRequested.current = true;
    slideStartX.current = null;
    slideDistanceRef.current = 0;
    setSlideDistance(0);
    change("cover");
    playInteractionSound("shutter", props.muted);
    setSaving(true);
    setCheckpointError(null);
    try {
      await props.onCovered?.();
      window.setTimeout(() => change("write"), reducedMotion ? 0 : 300);
    } catch {
      coverRequested.current = false;
      setState("look");
      setCheckpointError("We couldn't freeze that cover step yet. Please try again.");
    } finally {
      setSaving(false);
    }
  }
  ...
  const displayWord = props.studyDisplay ?? (props.splitPoints.length === 0 ? props.word : <>{parts.map((part, index) => <span key={`${part}-${index}`}>{index > 0 ? <span className="mx-1 text-cyan-300">|</span> : null}{part}</span>)}</>);

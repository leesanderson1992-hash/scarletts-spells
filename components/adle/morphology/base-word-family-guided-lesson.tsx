...
export function BaseWordFamilyGuidedLesson(props: {
  ...
  return <WordLabScene beat={guideBeat(state.stage)} phase={phase} muted={muted} onMutedChange={setMuted} silent={state.stage === "controlled" || state.stage === "dictation"} help={clueOpen ? clueFor(state.stage) : undefined} onHelp={() => setClueOpen((current) => !current)} guideName="Word Builder">
    ...
    {state.stage === "reflect" ? <Reflection payload={props.payload} sentenceAttempts={state.sentenceAttempts} prompt={props.payload.reflectionPrompt} value={state.reflectionText} completionLabel={props.onComplete ? "Finish Word Lab" : "Finish preview"} onValue={(reflectionText) => update({ reflectionText })} onComplete={() => { props.onPreviewComplete?.(state.reflectionText); props.onComplete?.({ reflection: state.reflectionText, controlledAttempts: state.controlledAttempts, sentenceAttempts: state.sentenceAttempts }); }} /> : null}
  </WordLabScene>;
}
...
function Reflection(props: { payload: BaseWordFamilyLessonSnapshotV1; sentenceAttempts: Record<string, string>; prompt: string; value: string; completionLabel: string; onValue: (value: string) => void; onComplete: () => void }) {
  ...
  return <section className="grid gap-5"><h2 className="text-center text-3xl font-black text-white">What did you notice?</h2><section className="rounded-3xl border border-white/15 bg-white/[.07] p-4 text-left" aria-label="Your word recap"><h3 className="text-lg font-black text-white">Your word recap</h3>{secure.length ? <div className="mt-3"><p className="text-sm font-black text-cyan-100">Words you spelled securely</p><p className="mt-1 text-cyan-50">{secure.map((outcome) => outcome.word.displayWord).join(", ")}</p></div> : null}{revisit.length ? <div className="mt-3"><p className="text-sm font-black text-amber-100">Words to look at again</p><p className="mt-1 text-cyan-50">{revisit.map((outcome) => outcome.word.displayWord).join(", ")}</p><p className="mt-1 text-sm text-cyan-100">Use the base word to help you notice the part that stays the same.</p></div> : null}</section><label className="text-base font-black text-white">{props.prompt}<textarea autoFocus required maxLength={2000} value={props.value} onChange={(event) => props.onValue(event.target.value)} className="mt-2 min-h-32 w-full rounded-2xl bg-white p-4 text-lg font-normal text-slate-950 focus:outline-none focus:ring-4 focus:ring-cyan-300/30" /></label><button type="button" disabled={!props.value.trim()} onClick={props.onComplete} className="min-h-12 rounded-full bg-cyan-300 font-black text-slate-950 disabled:opacity-40">{props.completionLabel}</button></section>;
}

...
function Cleave(props: { word: BaseWordFamilySnapshotWord | undefined; step: number; misses: Record<string, number>; onMiss: (id: string, misses: number) => void; onNext: (nextStep: number) => void }) {
  ...
  if (!valid) return <section className="grid gap-5 text-center"><p className="text-xs font-black uppercase tracking-[.2em] text-cyan-200">Find the word parts</p><h2 className="text-3xl font-black text-white">This word needs a different way to explore its parts.</h2><p className="text-cyan-50">Let’s carry on with the rest of the Word Lab.</p><button type="button" onClick={() => props.onNext(2)} className="mx-auto min-h-12 rounded-full bg-cyan-300 px-7 font-black text-slate-950">Build words from meanings</button></section>;
  const requiredCuts = (baseIndex > 0 ? 1 : 0) + (baseIndex < wordParts.length - 1 ? 1 : 0);
  if (props.step >= requiredCuts) return <section className="grid gap-5 text-center"><p className="text-cyan-50">Your base word is ready to use.</p><button type="button" onClick={() => props.onNext(requiredCuts)} className="mx-auto min-h-12 rounded-full bg-cyan-300 px-7 font-black text-slate-950">Build words from meanings</button></section>;
  const boundary = baseIndex > 0 && props.step === 0 ? baseIndex - 1 : baseIndex;
  const key = `${word!.canonicalWordId}:base:${boundary}`;
  return <BaseWordCleaver key={`${word!.canonicalWordId}:${props.step}`} word={word!.displayWord} segments={wordParts.map((part) => ({ id: part.id, text: part.surfaceText }))} baseIndex={baseIndex} completedCuts={props.step} misses={props.misses[key] ?? 0} onMiss={(misses) => props.onMiss(key, misses)} onContinue={() => props.onNext(props.step + 1)} />;
}

...
const profile: DynamicAffixProfile = { microSkillKey: "D4_MOR_SUFFIXES_NESS", position: "after", productionEnabled: true, affixLabel: "-ness", affixText: "ness", affixMeaning: "state or quality", meaningBins: [{ id: "state", label: "STATE OR QUALITY", description: "a state or quality" }], includeMeaningSort: false, wordsByCanonicalId: new Map(words.map((entry) => [entry.canonicalWordId, entry])), choices: [{ text: "ness", label: "-ness", outcome: null, meaning: "state or quality", status: "target" }, { text: "nes", label: "-nes", outcome: null, meaning: null, status: "unsupported" }, { text: "niss", label: "-niss", outcome: null, meaning: null, status: "unsupported" }], reflection: { promptKey: "ness-notice", promptText: "What did -ness do to the word?" }, introduction: { title: "What is a suffix?", paragraphs: ["A suffix is added to the end of a word."], spellingRules: ["-ness is spelled n-e-s-s."], examples: [{ affix: "ness", base: "kind", word: "kindness", meaning: "the state of being kind" }] } };
...
const ableIbleProfile: DynamicAffixProfile = {
  ...
  wordsByCanonicalId: new Map(ableIbleWords.map((entry) => [entry.canonicalWordId, entry])),
...
const incompleteSelection = selectDynamicAffixWordLab({ profiles: [incomplete], learningItems: [ableIbleItem] });
assert.equal(incompleteSelection ? compileDynamicAffixWordLabPayload(incompleteSelection) : null, null, "incomplete reviewed dictation fails before compilation");

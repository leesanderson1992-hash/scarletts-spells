...
const selected = {
  id: THERE_V2_MANIFEST.releaseId, release_key: THERE_V2_MANIFEST.releaseKey, family_key: THERE_V2_MANIFEST.familyKey,
...
for (const [index, manifest] of CONTEXT_FAMILY_MANIFESTS.entries()) {
  const releaseSuffix = String(index + 1).padStart(12, "0");
  const legacy = { id: `81000000-0000-4000-8000-${releaseSuffix}`, release_key: `s8-v1-${manifest.familyKey.toLowerCase().replaceAll("_", "-")}`, family_key: manifest.familyKey, analyser_version: WHOLE_WRITING_CONTEXT_ANALYSER_VERSION,

...
const routeActivityMarkdown = [
  ...
  ...SHARED_AFFIX_PROFILE_REGISTRY.map((profile) =>
    `| ${profile.microSkillKey} | ${profile.routeId}:${profile.routeVersion} | ${profile.position} | ${profile.forms.join(", ")} | ${profile.policy.expectedAssignmentItemCount} | ${profile.policy.split.kind} | ${profile.policy.build.kind} | ${profile.policy.meaning.kind} |`,
  ),
  "",
  "## Blockers",
  "",
...
const outputs = new Map<string, string>([
  ...
  [
    ...
    json({
      generatedNotice: GENERATED_NOTICE,
      activationAuthority: "none_shadow_only",
      ...
      parityRegression: "scripts/adle-shared-affix-production-parity-regression.ts",
      stagingProof: "scripts/adle-shared-affix-staging-proof.ts",
    }),
  ],
  [
    ...
    json({
      generatedNotice: GENERATED_NOTICE,
      compilerVersion: SHARED_AFFIX_COMPILER_VERSION,
      blockers: SHARED_AFFIX_BLOCKER_CODES,
    }),
  ],
  [

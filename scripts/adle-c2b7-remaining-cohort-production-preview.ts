...
import { buildCutoverPreview, type CutoverPreviewRecord } from "../lib/adle/review-policy/cutover-preview";
...
const READ_ONLY_CONFIRMATION = `ADLE-C2B7-REMAINING-COHORT-READ-ONLY:${PRODUCTION_PROJECT_REF}`;
const APPROVED_CHILD_ID = "e4f9fc37-3f85-4eb5-9fbd-4eabf4f2528e";
const CANARY_SCHEDULE_WORD_ID = "5d5e843f-df5d-4188-ae53-65158b02021d";
const CANARY_REVIEW_SESSION_ID = "71865eb0-8ecd-5141-9550-da761dc2d4a2";
const SOURCE_REVIEWED_PREVIEW_FINGERPRINT =
...
const PREVIEW_VERSION = "ADLE_C2B7_REMAINING_LEARNER_COHORT_PREVIEW_V1";
...
function fail(message: string): never {
  throw new Error(`C2B.7 remaining-cohort Production preview refused: ${message}`);
}
...
type ReviewOutcomeLineage = {
  id: string;
  schedule_word_id: string;
  due_kind: string;
  event_type: string;
  original_result: string;
  frozen_due_on: string;
...
select
  ...
  (select count(*)::text from public.adle_review_outcome_events) as outcome_count,
  (select encode(digest(coalesce(string_agg(to_jsonb(outcome)::text, E'\\n' order by outcome.id::text), ''), 'sha256'), 'hex')
    from public.adle_review_outcome_events outcome) as outcome_fingerprint
`;
...
const CANARY_REVIEW_OUTCOMES_SQL = `
select id::text, schedule_word_id::text, due_kind, event_type, original_result,
       frozen_due_on::text, frozen_interval_index::integer,
       review_completed_on::text, completed_at::text
from public.adle_review_outcome_events
where review_session_id = '${CANARY_REVIEW_SESSION_ID}'::uuid
order by schedule_word_id::text, id::text
`;
...
function exactCandidateProjection(record: CutoverPreviewRecord, previewFingerprint: string) {
  const approved = buildApprovedCutoverCandidate({
    record,
    reviewedPreviewFingerprint: previewFingerprint,
    approvalReference: APPROVAL_REFERENCE,
  });
  return {
    scheduleWordId: record.scheduleWordId,
    canonicalWordId: record.canonicalWordId,
    current: record.current,
    resultingState: record.proposed,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const reviewOutcomes = await select<ReviewOutcomeLineage>(client, CANARY_REVIEW_OUTCOMES_SQL);
    if (policyRows.length !== 1) fail("exact target policy registry row is missing or duplicated");
    if (policyRows[0].is_active !== false || policyRows[0].is_default_for_new_schedules !== false) {
      fail("target registry is active or default");
    }
    ...
    const globalPreview = buildCutoverPreview({
      rows,
      legacyBundles,
      targetPolicy: policyRows[0],
      asOfDate: previewAsOfDate,
    });
    const learnerRows = rows.filter((row) => row.child_id === APPROVED_CHILD_ID);
    const learnerPreview = buildCutoverPreview({
      rows: learnerRows,
      legacyBundles,
      targetPolicy: policyRows[0],
      asOfDate: previewAsOfDate,
    ...
    const canary = learnerPreview.records.find((record) => record.scheduleWordId === CANARY_SCHEDULE_WORD_ID);
    if (!canary || canary.reason !== "ALREADY_TARGET_V2") fail("canary is not the exact existing target-v2 exclusion");
    if (reviewOutcomes.length !== 10) fail(`expected 10 governed canary Review outcomes, found ${reviewOutcomes.length}`);
    const resolvedCatchUpOutcomes = reviewOutcomes.filter((outcome) =>
      outcome.due_kind === "catch_up_retest" && outcome.original_result === "success");
    if (resolvedCatchUpOutcomes.length !== 3) {
      fail(`expected exactly 3 successful catch-up outcomes, found ${resolvedCatchUpOutcomes.length}`);
    }
    const resolvedCatchUpIds = new Set(resolvedCatchUpOutcomes.map((outcome) => outcome.schedule_word_id));
    if (resolvedCatchUpIds.size !== 3) fail("successful catch-up outcomes do not identify exactly 3 schedule words");

    const scheduledFailureOutcomes = reviewOutcomes.filter((outcome) =>
      outcome.due_kind === "scheduled_review"
      && outcome.original_result === "failure"
      && outcome.schedule_word_id !== CANARY_SCHEDULE_WORD_ID);
    if (scheduledFailureOutcomes.length !== 1) {
      fail(`expected exactly 1 non-canary scheduled failure, found ${scheduledFailureOutcomes.length}`);
    }
    const driftedOriginal = learnerPreview.records.find(
      (record) => record.scheduleWordId === scheduledFailureOutcomes[0].schedule_word_id,
    );
    if (!driftedOriginal || driftedOriginal.reason !== "CATCH_UP_EPISODE_UNRESOLVED") {
      fail("the scheduled failure is not preserved as an unresolved catch-up exclusion");
    }

    const currentEligible = learnerPreview.records.filter((record) => record.eligibility === "ELIGIBLE");
    const newlyEligible = currentEligible.filter((record) => resolvedCatchUpIds.has(record.scheduleWordId));
    if (newlyEligible.length !== 3) fail("the 3 resolved catch-up schedules are not all newly eligible now");
    const remaining = currentEligible
      .filter((record) => !resolvedCatchUpIds.has(record.scheduleWordId))
      .sort((left, right) => left.scheduleWordId.localeCompare(right.scheduleWordId));
    if (remaining.length !== 17) {
      fail(`reviewed original cohort cannot be reconstructed exactly: expected 17 still eligible, found ${remaining.length}`);
    }
    if (remaining.some((record) => record.childId !== APPROVED_CHILD_ID)) fail("another learner entered the cohort");

    const normalized = {
      previewVersion: PREVIEW_VERSION,
      asOfDate: previewAsOfDate,
      productionProjectRef: PRODUCTION_PROJECT_REF,
      learnerId: APPROVED_CHILD_ID,
      approvalReference: APPROVAL_REFERENCE,
      sourceReviewedPreviewFingerprint: SOURCE_REVIEWED_PREVIEW_FINGERPRINT,
      currentGlobalPreviewFingerprint: globalPreview.fingerprint,
      currentLearnerPreviewFingerprint: learnerPreview.fingerprint,
      targetRegistry: {
        isActive: policyRows[0].is_active,
        isDefaultForNewSchedules: policyRows[0].is_default_for_new_schedules,
      },
      cohortProof: {
        sourceReviewedEligibleCount: 19,
        alreadyCutOverCanary: {
          scheduleWordId: canary.scheduleWordId,
          canonicalWordId: canary.canonicalWordId,
          current: canary.current,
          reason: canary.reason,
        },
        driftedOriginalCohortWord: {
          scheduleWordId: driftedOriginal.scheduleWordId,
          canonicalWordId: driftedOriginal.canonicalWordId,
          current: driftedOriginal.current,
          sourceOutcome: scheduledFailureOutcomes[0],
          excludedReason: "SOURCE_REVIEWED_WORD_DRIFTED_TO_CATCH_UP_AFTER_SCHEDULED_FAILURE",
        },
        newlyEligibleAfterCanaryReview: newlyEligible
          .sort((left, right) => left.scheduleWordId.localeCompare(right.scheduleWordId))
          .map((record) => ({
            scheduleWordId: record.scheduleWordId,
            canonicalWordId: record.canonicalWordId,
            sourceOutcome: resolvedCatchUpOutcomes.find(
              (outcome) => outcome.schedule_word_id === record.scheduleWordId,
            ),
            excludedReason: "NEWLY_ELIGIBLE_AFTER_REVIEW_NOT_IN_SOURCE_REVIEWED_COHORT",
          })),
        currentLearnerEligibleCount: currentEligible.length,
        remainingSourceReviewedAndStillEligibleCount: remaining.length,
        reconstructedSourceReviewedEligibleCount:
          1 + remaining.length + 1,
      },
      remainingRecords: remaining,
    };
    const fingerprint = fingerprintSnapshotValue(normalized);
    const candidates = remaining.map((record) => exactCandidateProjection(record, fingerprint));
    ...
    console.log(JSON.stringify({
      mode: "production_repeatable_read_read_only_remaining_learner_cohort_preview",
      projectRef: PRODUCTION_PROJECT_REF,
      transactionReadOnly: true,
      preview: { ...normalized, fingerprint, candidates },

...
assert(meaningSource.includes("useId().replaceAll") && meaningSource.includes('markerEnd={`url(#${markerId})`}') && meaningSource.includes("onPointerMove={pointer}"), "smoke gate: the meaning board must show a live cursor-following arrow and a snapped arrow head");
assert(lessonSource.includes('concept: "DICTATION"') && lessonSource.includes('mode: "target_span"') && sentenceDictationSource.includes("Write the whole sentence") && sentenceDictationSource.includes("Check sentence") && sentenceDictationSource.includes("autoFocus") && sentenceDictationSource.includes('mode="sentence"'), "smoke gate: Compound must configure canonical sentence dictation with the existing editable input and check/reveal flow");
...
assert(lessonSource.includes('concept: "LESSON_REFLECTION"') && lessonSource.includes("EXACT_GOVERNED_FORM_ANSWER_POLICY") && lessonSource.includes("lessonReflectionSentenceComparison"), "smoke gate: Compound normalizes exact-governed misses and feedback-only sentence comparisons into LessonReflection");

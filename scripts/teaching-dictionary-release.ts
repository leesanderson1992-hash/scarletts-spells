...
async function buildTableRows(
  ...
  const dictations = dictationRows.map((row, index) => ({
    ...
    ...(Object.hasOwn(row, "dictation_target_end_exclusive") ? {
      dictation_target_end_exclusive: Number.parseInt(row.dictation_target_end_exclusive, 10),
      exact_governed_answer: row.exact_governed_answer,
    } : {}),

...
const profile: DynamicAffixProfile = { microSkillKey: reviewed.profile.microSkillKey, position: "after", productionEnabled: true, affixLabel: "-ity", affixText: "ity", affixMeaning: "the state or quality of being", meaningBins: reviewed.profile.meaningBins, includeMeaningSort: false, wordsByCanonicalId: new Map(words.map((word) => [word.canonicalWordId, word])), transferCanonicalWordIds: words.map((word) => word.canonicalWordId), choices: reviewed.profile.suffixChoices, reflection: reviewed.profile.reflection, introduction: reviewed.profile.introContent };
...
const rosterProfile = { ...profile, wordsByCanonicalId: new Map([...profile.wordsByCanonicalId, [future.canonicalWordId, future]]), transferCanonicalWordIds: [...profile.transferCanonicalWordIds, future.canonicalWordId] };

...
const PROFILE = "D4_MOR_SUFFIXES_TION"; const EMAIL = "katiesanderson8624@gmail.com"; const HOST = "jlhotktspjvffslvuyfz.supabase.co";
...
async function main() {
  ...
  if (profileError || !profile || profile.production_enabled) throw new Error(profileError?.message ?? "The staging-only -tion profile is unavailable.");
  ...
  if (members.length !== 4) throw new Error("Expected exactly four reviewed -tion members.");
  const { data: word, error: wordError } = await db.from("canonical_teaching_dictionary_words").select("id").eq("normalised_word", "invention").eq("row_status", "active").eq("review_status", "approved_for_first_exposure").single();
  if (wordError || !word || !members.some((member: any) => member.canonical_word_id === word.id)) throw new Error(wordError?.message ?? "Missing invention profile member.");
  const { data: child, error: childError } = await db.from("children").insert({ parent_user_id: parent.id, first_name: "ADLE -tion verification" }).select("id").single();
  if (childError || !child) throw new Error(childError?.message ?? "Could not create verification child.");
  const { error: itemError } = await db.from("adle_learning_items").insert({ child_id: child.id, canonical_word_id: word.id, micro_skill_key: PROFILE, item_status: "pending", source_kind: "verified_misspelling", source_ref: `owner-verification:${child.id}`, source_attempt_text: "invenshun", reteach_priority: false, intake_on: new Date().toISOString().slice(0, 10), row_status: "active" });

...
const candidateRelease = process.argv.includes("--its-v2");
...
if (requestedFamilyIndex >= 0 && (!requestedFamily || requestedFamily.startsWith("--"))) throw new Error("--family requires one family key");
const knownArguments = new Set(["--its-v2", "--family", ...(requestedFamily ? [requestedFamily] : [])]);
const unknownArguments = process.argv.slice(2).filter((argument) => !knownArguments.has(argument));
if (unknownArguments.length) throw new Error(`Unknown evaluation option: ${unknownArguments.join(", ")}`);
if (candidateRelease && requestedFamily && requestedFamily !== ITS_V2_MANIFEST.familyKey) throw new Error("--its-v2 can evaluate only ITS_ITS");
const selectedFamilyManifests = candidateRelease
  ? CONTEXT_FAMILY_MANIFESTS.filter((manifest) => manifest.familyKey === ITS_V2_MANIFEST.familyKey)
  : requestedFamily
...
const outputRoot = candidateRelease ? join(packageRoot, "release-evaluations", ITS_V2_MANIFEST.releaseKey) : packageRoot;
const releasePin = candidateRelease
  ? JSON.parse(readFileSync(join(outputRoot, "release.json"), "utf8")) as {
    fingerprint: string;
    manifestFingerprint: string;
    sourceSha256: string;
    manifest: typeof ITS_V2_MANIFEST;
    packageFingerprint: string;
...
if (releasePin) {
  ...
  if (releasePin.manifestFingerprint !== ITS_V2_MANIFEST_FINGERPRINT || recordFingerprint(releasePin.manifest) !== recordFingerprint(ITS_V2_MANIFEST)) throw new Error("Candidate manifest mismatch");
  if (releasePin.sourceSha256 !== sha256(readFileSync(join(repositoryRoot, "lib/writing-engine/whole-writing/context-its-v2.ts")))) throw new Error("Candidate analyser source mismatch");
...
for (const familyManifest of selectedFamilyManifests) {
  ...
  const evaluation = evaluateFamily({
    ...
    analyser: candidateRelease ? analyseItsContextV2 : undefined,
  ...
  const report = {
    ...
    provenance: {
      ...
      ...(releasePin ? {
        lockedCorpusPackageFingerprint: packageManifest.packageFingerprint,
        lockedFiles: releasePin.lockedFiles,
        originalRuntime: packageManifest.runtime,
      } : {}),
    },
    variety,
    metrics: evaluation,

...
const PACKAGE = resolve(ROOT, "docs/implementation/seed-data/teaching-dictionary/candidates/2026-07-27-dynamic-suffix-ness/reviewed-staging-package.json");
const fail = (message: string): never => { throw new Error(`Dynamic suffix staging import refused: ${message}`); };
...
async function main() {
  if (process.argv.includes("--validate")) { validate(JSON.parse(readFileSync(PACKAGE, "utf8"))); console.log("Dynamic suffix package validation passed."); return; }
  if (!process.argv.includes("--environment") || process.argv[process.argv.indexOf("--environment") + 1] !== "staging") fail("pass --environment staging");
  ...
  if (!url) fail("ADLE_DYNAMIC_SUFFIX_STAGING_DATABASE_URL is required");
  ...
  try {
    ...
    const words = await client.query("select w.id,w.normalised_word from canonical_teaching_dictionary_words w where w.normalised_word = any($1) and w.row_status='active' and w.review_status='approved_for_first_exposure'", [pkg.words.map((word: any) => word.word)]);
    if (words.rowCount !== pkg.words.length) fail("every reviewed package word must already be an active approved canonical word");
    ...
    if (JSON.stringify(protectedBefore.rows[0]) !== JSON.stringify(protectedAfter.rows[0])) fail("protected learner, assignment, evidence, or scheduling data changed");
    await client.query("commit"); console.log(JSON.stringify({ status: "imported_and_verified", packageSha256: packageHash, productionEnabled: false, profiles: 1, members: pkg.words.length }));
  } catch (error) { await client.query("rollback"); throw error; } finally { await client.end(); }
}
function validate(pkg: any) {
  if (pkg?.schemaVersion !== 1 || pkg.profile?.microSkillKey !== "D4_MOR_SUFFIXES_NESS" || pkg.profile.includeMeaningSort !== false || pkg.profile.meaningBins?.length !== 1 || !Array.isArray(pkg.words) || pkg.words.length < 4) fail("invalid NESS package envelope");
  for (const word of pkg.words) { const teaching = word.teaching?.parts ?? []; const trueParts = word.trueMorphology?.parts ?? []; const suffix = teaching.filter((part: any) => part.kind === "suffix"); if (suffix.length !== 1 || suffix[0].surfaceText !== word.suffixVariant || teaching.map((part: any) => part.surfaceText).join("") !== word.word || trueParts.map((part: any) => part.surfaceText).join("") !== word.word || !word.trueMorphology?.provenance || tokenAt(word.dictation?.sentence ?? "", word.dictation?.targetTokenIndex) !== word.word || word.dictation.audioText !== word.dictation.sentence) fail(`invalid reviewed facts for ${word.word ?? "unknown word"}`); }
}

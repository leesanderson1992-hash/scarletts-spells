#!/usr/bin/env python3
"""Build a canonical-word readiness batch, without prescribing a lesson route.

MorphoLex is retained as linguistic evidence.  A reviewer, not this builder,
decides whether a child-facing word sum and transfer features are safe to use.
"""
...
EXISTING_REPAIRS = [("govern", "metadata_add"), ("governor", "metadata_add"), ("tall", "metadata_add")]
...
CSV_HEADERS = {
    ...
    "teaching_content_versions.csv": ["micro_skill_key", "content_version", "version_status", "is_active", "teaching_objective", "reflection_prompt_key", "reflection_prompt_text", "child_friendly_explanation", "rule_explanation", "memory_tip", "common_misconceptions", "first_exposure_progression", "guided_practice_progression", "review_proofreading_progression", "example_selection_guidance", "contrast_policy_guidance", "sample_preview_word_key", "source_category", "source_name", "source_url", "source_licence", "source_use_note", "confidence", "supersedes_content_version", "final_readiness_review_status", "final_readiness_reviewed_by", "final_readiness_reviewed_at"],
...
def main() -> None:
    ...
    words_rows, metadata_rows, morphology_rows, dictation_rows, register = [], [], [], [], []
    for rank, row in enumerate(selected, 1):
        ...
        morphology_rows.append({"word_key": word_key, "raw_morpholex_segmentation": m.get("segmentation", ""), "raw_morpholex_pos": m.get("pos", ""), "morphology_parts": "[]", "feature_keys": "[]", "morphology_joins": "[]", "transformation_notes": "", "word_sum": "", "analysis_status": "in_review", "source_category": "open_licensed" if morphology_verified else "internal_authored", "source_name": "MorphoLex-en" if morphology_verified else "No exact MorphoLex-en row", "source_url": "https://github.com/hugomailhot/MorphoLex-en" if morphology_verified else "", "source_licence": "CC BY-NC-SA 4.0" if morphology_verified else "", "source_use_note": "Raw linguistic candidate only. A reviewer must decide the structured analysis and child-facing word sum; do not infer either from this row.", "confidence": "medium" if morphology_verified else "low", "review_status": "in_review", "reviewed_by": "", "reviewed_at": "", "review_notes": ""})
        ...
        register.append({"rank": rank, "word_key": word_key, "word": word, "display_word": display, "learner_demand_selection_evidence": row["forced"], "curriculum_evidence": True, "curriculum_age": row["curriculum_age"], "aoa_median": row["aoa"] or "", "wordfreq_zipf": row["zipf"], "frequency_band": frequency_band(row["zipf"]), "bnc_frequency": row["bnc_frequency"], "starter_source_continuity": row["prior_source"], "morpholex_exact": morphology_verified, "raw_morpholex_segmentation": m.get("segmentation", ""), "british_ipa": ipa_value, "cmudict": cmu_value, "syllables": syllables, "age_band": age_band(age_value), "complexity_band": complexity, "identity_review": "in_review", "selection_evidence_review": "in_review", "pronunciation_review": "in_review", "british_english_review": "in_review", "accessibility_review": "in_review", "source_licence_review": "in_review", "final_decision": "in_review", "reviewed_by": "", "reviewed_at": "", "review_notes": ""})

    csv_dir = args.output / "csv"
    payloads = {"canonical_words.csv": words_rows, "canonical_word_metadata.csv": metadata_rows, "canonical_word_morphology.csv": morphology_rows, "dictation_sentences.csv": dictation_rows, "teaching_content_versions.csv": [], "teaching_content_field_reviews.csv": [], "teaching_content_sources.csv": [
    ...
    repair_rows = [{"word": word, "repair_type": repair, "review_status": "in_review", "reviewed_by": "", "reviewed_at": "", "review_notes": ""} for word, repair in EXISTING_REPAIRS]
    write_csv(args.output / "review" / "existing_row_repairs.csv", list(repair_rows[0]), repair_rows)

    morphology_review = [{**row, "linguistic_analysis_review": "in_review", "word_sum_review": "in_review", "final_decision": "in_review"} for row in morphology_rows]
    ...
    write_csv(args.output / "review" / "canonical_word_review.csv", review_headers, register)
    write_csv(args.output / "review" / "linguistic_morphology_word_sums_review.csv", list(morphology_review[0]), morphology_review)
    ...
    workbook_data = {
        "Overview & instructions": [["Topic", "Instruction"], ["Scope", "Canonical-word readiness only. No word is assigned to a micro-skill in this batch."], ["Policy", "Capability ready, not pre-diagnosed: a real approved correction later activates a signed-off micro-skill."], ["MorphoLex", "Linguistic evidence only. Do not turn its segmentation into a child-facing word sum automatically."], ["Word sums", "Approve a teachable word sum, mark not applicable, or reject the analysis. Never leave a morphology decision unresolved."], ["Transfer selection", "Future ADLE uses approved feature records and an active selector profile; authentic use comes from the learning item."], ["Status", "HUMAN REVIEW REQUIRED"]],
        "Canonical word review": [review_headers] + [[row[h] for h in review_headers] for row in register],
        "Linguistic morphology & word sums": [list(morphology_review[0])] + [[row[h] for h in morphology_review[0]] for row in morphology_review],
        "Dictation review": [list(dictation_review[0])] + [[row[h] for h in dictation_review[0]] for row in dictation_review],
        "Existing word-fact repairs": [list(repair_rows[0])] + [[row[h] for h in repair_rows[0]] for row in repair_rows],
        "Sources & licence": [list(source_review[0])] + [[row.get(h, "") for h in source_review[0]] for row in source_review],
    }
    (args.output / "review-workbook-data.json").write_text(json.dumps(workbook_data, ensure_ascii=False), encoding="utf-8")
    manifest = {"schemaVersion": "canonical_word_readiness_v1", "batchType": "canonical_word_only", "batchSize": len(selected), "newWordRows": len(words_rows), "metadataRows": len(metadata_rows), "morphologyRows": len(morphology_rows), "supportRows": 0, "dictationRows": len(dictation_rows), "existingRepairRows": len(repair_rows), "status": "human_review_required", "productionMutationAuthorised": False, "sourceSnapshot": str(args.snapshot), "selectionSha256": hashlib.sha256("\n".join(row["word"] for row in selected).encode()).hexdigest()}

...
const makeWord = (id: string, displayWord: string) => {
  ...
  return {
    ...
    parts: [
      ...
      {
        id: `${id}:b`,
        text: displayWord.slice(2),
        sourceText: displayWord.slice(2),
        role: "base" as const,
        start: 2,
...
const profile: DynamicPrefixProfile = {
  ...
  reflection: { promptKey: "test", promptText: "Notice it." },
};
const authentic = {
  learningItemId: "li-authentic",
  childId: "child",
  canonicalWordId: "a",
...
const base = {
  childId: "child",
  planDate: "2026-07-21",
  composerPolicyVersion: "c",
  schedulePolicyVersion: "s",
  throttle: {},
  partOne: {},
  partTwo: {},
  budget: {
...
assert.equal(
  all.find((item) => item.payload.dynamicPrefixActivityId === "intro-root")
    ?.payload.dynamicPrefixLesson,
  payload,
);
const production = plan.partTwo.sections.find(
  (section) => section.sectionKey === "lesson_production",
)!.items;
assert.equal(
  production.filter((item) => item.learningItemId !== null).length,
  1,
);
assert.equal(
  production.filter((item) => item.learningItemId === null).length,
...
console.log(
  "PASS: Dynamic Prefix v2 plan persists four words and links authentic targets only",
);

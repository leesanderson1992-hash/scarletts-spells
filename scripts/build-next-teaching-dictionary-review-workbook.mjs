...
for (const [sheetName, matrix] of Object.entries(data)) {
  ...
  if (sheetName === "Overview & instructions") {
    ...
    sheet.freezePanes.freezeColumns(sheetName === "Canonical word review" ? 3 : 1);
    range.format.autofitColumns();
    for (let col = 0; col < width; col += 1) {
      const column = sheet.getRangeByIndexes(0, col, normalized.length, 1);
      const headerValue = String(normalized[0][col] ?? "");
      if (/notes|source|segmentation|morphology|feature|word_sum|cmudict|ipa|sentence|audio_text/i.test(headerValue)) {
    ...
    for (const field of headers.filter((field) => field.endsWith("_review") || field === "final_decision" || field === "review_status" || field === "analysis_status")) {
      const index = headers.indexOf(field);
      if (index < 0 || normalized.length < 2) continue;
      const editable = sheet.getRangeByIndexes(1, index, normalized.length - 1, 1);
      const values = field === "analysis_status" ? ["in_review", "approved", "not_applicable", "rejected"] : ["in_review", "approved", "changes_requested", "rejected"];
      editable.dataValidation = { rule: { type: "list", values } };
...
const selectionCheck = await workbook.inspect({
  kind: "table",
  range: "Canonical word review!A1:H8",
...
if (previewDir) {
  ...
  for (const sheetName of Object.keys(data)) {
    ...
    const previewRows = Math.min(matrix.length, sheetName === "Canonical word review" ? 25 : 40);
    const previewCols = Math.min(Math.max(...matrix.map((row) => row.length)), sheetName === "Canonical word review" ? 12 : 20);
    const endColumn = (() => {

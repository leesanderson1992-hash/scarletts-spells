...
const evidenceRepair = readFileSync("supabase/migrations/20260812180000_route_release_bound_evidence_through_shared_adle_lineage.sql", "utf8");
assert(evidenceRepair.includes("item.metadata->>'adleLearningItemRef'"));
assert(evidenceRepair.includes("join public.adle_learning_items learning"));
assert(evidenceRepair.includes("v_definition:=replace(v_definition,v_legacy,v_shared)"), "forward migration removes the legacy lineage projection");

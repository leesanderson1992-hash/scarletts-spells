/** Creates a child-review staging lesson only; production is never contacted. */
/* eslint-disable @typescript-eslint/no-explicit-any */
import { createClient } from "@supabase/supabase-js";

const PROFILE = "D4_MOR_SUFFIXES_SION"; const EMAIL = "katiesanderson8624@gmail.com"; const HOST = "jlhotktspjvffslvuyfz.supabase.co";
...
async function main() {
  ...
  const { data: users, error: userError } = await db.auth.admin.listUsers({ page: 1, perPage: 1000 }); const parent = users?.users.find((user) => user.email?.toLowerCase() === EMAIL);
  ...
  if (profileError || !profile || profile.production_enabled) throw new Error(profileError?.message ?? "The staging-only -sion profile is unavailable.");
  ...
  if (members.length !== 4) throw new Error("Expected exactly four reviewed -sion members.");
  const { data: word, error: wordError } = await db.from("canonical_teaching_dictionary_words").select("id").eq("normalised_word", "decision").eq("row_status", "active").eq("review_status", "approved_for_first_exposure").single();
  if (wordError || !word || !members.some((member: any) => member.canonical_word_id === word.id)) throw new Error(wordError?.message ?? "Missing decision profile member.");
  const { data: child, error: childError } = await db.from("children").insert({ parent_user_id: parent.id, first_name: "ADLE -sion verification" }).select("id").single();
  if (childError || !child) throw new Error(childError?.message ?? "Could not create verification child.");
  const { error: itemError } = await db.from("adle_learning_items").insert({ child_id: child.id, canonical_word_id: word.id, micro_skill_key: PROFILE, item_status: "pending", source_kind: "verified_misspelling", source_ref: `owner-verification:${child.id}`, source_attempt_text: "decishun", reteach_priority: false, intake_on: new Date().toISOString().slice(0, 10), row_status: "active" });

...
const PROFILE = "D4_MOR_SUFFIXES_ITY"; const EMAIL = "katiesanderson8624@gmail.com"; const HOST = "jlhotktspjvffslvuyfz.supabase.co";
...
async function main() {
  ...
  if (profileError || !profile || profile.production_enabled) throw new Error(profileError?.message ?? "The staging-only -ity profile is unavailable.");
  ...
  if (members.length < 4) throw new Error("Expected at least four reviewed -ity members.");
  const { data: word, error: wordError } = await db.from("canonical_teaching_dictionary_words").select("id").eq("normalised_word", "equality").eq("row_status", "active").eq("review_status", "approved_for_first_exposure").single();
  if (wordError || !word || !members.some((member: any) => member.canonical_word_id === word.id)) throw new Error(wordError?.message ?? "Missing equality profile member.");
  const { data: child, error: childError } = await db.from("children").insert({ parent_user_id: parent.id, first_name: "ADLE -ity verification" }).select("id").single();

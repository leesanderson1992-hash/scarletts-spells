...
The amended `THERE_THEIR_THEYRE` workbook is preserved byte-for-byte under `human-review/primary-label/raw`. Its 52 supplemental passages add 107 governed occurrences: 57 `THERE_THEIR_THEYRE` and 50 `TO_TOO_TWO`. The human target decisions cover 52 `THERE_THEIR_THEYRE` occurrences. Complete the two files under `human-review/primary-label/pending` for the five remaining `THERE_THEIR_THEYRE` occurrences and 50 incidental `TO_TOO_TWO` occurrences. Do not copy target decisions onto neighbouring occurrences.
